...
parser.add_argument('--count', type=int, default=300) # todo: K2 100 300
parser.add_argument('--batch_size', default=100)
parser.add_argument('--unlabeled_weight', type=float, default=1.)
parser.add_argument('--learning_rate', type=float, default=0.0003)
parser.add_argument('--break_on_epoch', type=int, default=201)
parser.add_argument('--net_scale_factor', type=int, default=8, # todo: 1 2 4 8 16 32
                    help="Major network dimensions will be _divided_ by this "
                         "number. Must be power of 2, between 1 and 32")
# todo optimization=None,
...
# load
from lib.datasource import wisdm_train_x, wisdm_train_y, wisdm_test_y, wisdm_test_x

# Currently we have (6000, 3, 100)
assert wisdm_train_x.shape == (6000, 3, 100)
trainx = np.reshape(wisdm_train_x, (-1, 3, 100, 1)).astype(th.config.floatX) / 21.0
testx = np.reshape(wisdm_test_x, (-1, 3, 100, 1)).astype(th.config.floatX) / 21.0
testy = np.argmax(wisdm_test_y, axis=1).astype(np.int32)
trainy = np.argmax(wisdm_train_y, axis=1).astype(np.int32)
...
gen_layers.append(nn.batch_norm(ll.DenseLayer(gen_layers[-1], num_units=13 * 1 * 512 / sf, W=Normal(0.05), nonlinearity=nn.relu), g=None))
gen_layers.append(ll.ReshapeLayer(gen_layers[-1], (args.batch_size, 512 / sf, 13, 1)))
gen_layers.append(nn.batch_norm(nn.Deconv2DLayer(gen_layers[-1], (args.batch_size, 256 / sf, 25, 1), (5, 1), stride=(2, 1), W=Normal(0.05), nonlinearity=nn.relu), g=None))
gen_layers.append(nn.batch_norm(nn.Deconv2DLayer(gen_layers[-1], (args.batch_size, 128 / sf, 50, 1), (5, 1), stride=(2, 1), W=Normal(0.05), nonlinearity=nn.relu), g=None))
gen_layers.append(nn.weight_norm(nn.Deconv2DLayer(gen_layers[-1], (args.batch_size, 3, 100, 1), (5, 1), stride=(2, 1), W=Normal(0.05), nonlinearity=T.tanh), train_g=True, init_stdv=0.1))
gen_dat = ll.get_output(gen_layers[-1])

# specify discriminative model
disc_layers = [ll.InputLayer(shape=(None, 3, 100, 1))]
disc_layers.append(ll.DropoutLayer(disc_layers[-1], p=0.2))
disc_layers.append(nn.weight_norm(dnn.Conv2DDNNLayer(disc_layers[-1], 96 / sf, (3, 1), pad=1, W=Normal(0.05), nonlinearity=nn.lrelu)))
disc_layers.append(nn.weight_norm(dnn.Conv2DDNNLayer(disc_layers[-1], 96 / sf, (3, 1), pad=1, W=Normal(0.05), nonlinearity=nn.lrelu)))
disc_layers.append(nn.weight_norm(dnn.Conv2DDNNLayer(disc_layers[-1], 96 / sf, (3, 1), pad=1, stride=(2, 1), W=Normal(0.05), nonlinearity=nn.lrelu)))
disc_layers.append(ll.DropoutLayer(disc_layers[-1], p=0.5))
disc_layers.append(nn.weight_norm(dnn.Conv2DDNNLayer(disc_layers[-1], 192 / sf, (3, 1), pad=1, W=Normal(0.05), nonlinearity=nn.lrelu)))
disc_layers.append(nn.weight_norm(dnn.Conv2DDNNLayer(disc_layers[-1], 192 / sf, (3, 1), pad=1, W=Normal(0.05), nonlinearity=nn.lrelu)))
disc_layers.append(nn.weight_norm(dnn.Conv2DDNNLayer(disc_layers[-1], 192 / sf, (3, 1), pad=1, stride=(2, 1), W=Normal(0.05), nonlinearity=nn.lrelu)))
disc_layers.append(ll.DropoutLayer(disc_layers[-1], p=0.5))
disc_layers.append(nn.weight_norm(dnn.Conv2DDNNLayer(disc_layers[-1], 192 / sf, (3, 1), pad=0, W=Normal(0.05), nonlinearity=nn.lrelu)))
disc_layers.append(nn.weight_norm(ll.NINLayer(disc_layers[-1], num_units=192 / sf, W=Normal(0.05), nonlinearity=nn.lrelu)))
disc_layers.append(nn.weight_norm(ll.NINLayer(disc_layers[-1], num_units=192 / sf, W=Normal(0.05), nonlinearity=nn.lrelu)))
disc_layers.append(ll.GlobalPoolLayer(disc_layers[-1]))
disc_layers.append(nn.weight_norm(ll.DenseLayer(disc_layers[-1], num_units=6, W=Normal(0.05), nonlinearity=None), train_g=True, init_stdv=0.1))
