...
parser.add_argument('--break_on_epoch', type=int, default=100)
parser.add_argument('--net_scale_factor', type=int, default=8,
                    help="Major network dimensions will be _divided_ by this "
                         "number. Must be power of 2, between 1 and 32")
parser.add_argument('--data_dir', type=str,
                    default='/Users/horban/tensorflow/cifar-10-python')
...
shape = (-1, 3, 32 * 32, 1)
trainx = trainx[:100].reshape(shape)
testx = testx[:100].reshape(shape)
trainy = trainy[:100]
testy = testy[:100]
...
gen_layers.append(nn.batch_norm(ll.DenseLayer(gen_layers[-1], num_units=32 * 32 / 8 * 1 * 512 / sf, W=Normal(0.05), nonlinearity=nn.relu), g=None))
gen_layers.append(ll.ReshapeLayer(gen_layers[-1], (args.batch_size, 512 / sf, 32 * 32 / 8, 1)))
gen_layers.append(nn.batch_norm(nn.Deconv2DLayer(gen_layers[-1], (args.batch_size, 256 / sf, 32 * 32 / 4, 1), (5, 1), stride=(2, 1), W=Normal(0.05), nonlinearity=nn.relu), g=None))  # 4 -> 8
gen_layers.append(nn.batch_norm(nn.Deconv2DLayer(gen_layers[-1], (args.batch_size, 128 / sf, 32 * 32 / 2, 1), (5, 1), stride=(2, 1), W=Normal(0.05), nonlinearity=nn.relu), g=None))  # 8 -> 16
gen_layers.append(nn.weight_norm(nn.Deconv2DLayer(gen_layers[-1], (args.batch_size, 3, 32 * 32, 1), (5, 1), stride=(2, 1), W=Normal(0.05), nonlinearity=T.tanh), train_g=True, init_stdv=0.1))  # 16 -> 32
gen_dat = ll.get_output(gen_layers[-1])

# specify discriminative model
disc_layers = [ll.InputLayer(shape=(None, 3, 32 * 32, 1))]
